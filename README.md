...
`slow` 在 ReAct 两头各加一个节点：开头 **planner** 先出一份调查提纲（只绑 `update_plan`
...
> 计划里写的是**要弄清楚什么**，不是**调哪个工具**。这条是实测逼出来的：最初提示词
> 要「可执行的步骤」，模型给出「列出所有文件 / 筛选 .py / 逐一读取」，然后照着执行，
> `list_dir` 被调了三遍，比不做规划还慢。改成"调查提纲"之后，同一个任务上 slow 的
> 工具调用反而比 fast 少、结论长 50%，代价是模型轮次多约一半。
>
