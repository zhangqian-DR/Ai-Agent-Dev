...
50.02s（**95.9%**），工具执行 11 次共 0.067s（**0.1%**，`read_file` 每次约 10ms）。
把工具改成理想并行最多省 0.053s——总时长的 0.1%。参考的那套架构里并行有意义，是因为
它的"工具"是网络 RPC（百毫秒级）；这里是本地文件读写，成本结构根本不同。真要压时间，
得动的是那 95.9%（并行的是**模型调用**，不是工具），那是另一个量级的设计。
